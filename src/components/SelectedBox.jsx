  ...
  return (
    <div>
      <div className="bg-gray-200 my-2 py-3 px-2 flex justify-between items-center rounded-md">
        <span className="text-white bg-sky-800 w-9 h-9 rounded-md flex justify-center items-center">{item[0].id}</span>
        <div>quantity :{item.length}</div>
        <div className="flex gap-2">
          <input type="checkbox" className="cursor-pointer" onChange={onChangeAll} checked={item.every(({ uniqueId }) => uniqueId in checked)} />
          <IoIosArrowDown
            fontSize={20}
            cursor="pointer"
      ...
      {isShowDetail &&
        item.map((todo) => (
          <div key={todo.uniqueId} className="flex justify-between items-center bg-slate-300 my-2 py-1 rounded-lg px-3">
            <span>#{todo.uniqueId}</span>
            <span>{todo.created.slice(0, 10)}</span>
            <input type="checkbox" className="cursor-pointer" checked={todo.uniqueId in checked} value={todo.uniqueId} onChange={onChange} />
          </div>
        ))}
    </div>
  );
