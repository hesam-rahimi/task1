...
function App() {
  ...
  const addHandler = () => {
    setLeftData((leftPrev) => {
      ...
      const newRightData = notGroupedData.filter((item) => item.uniqueId in checked);
      const newLeftData = notGroupedData.filter((item) => !(item.uniqueId in checked));
  ...
  return (
    <div className="bg-gray-600 min-h-screen w-full py-8 px-8 flex">
      ...
      <div className="w-1/2 bg-white rounded-xl py-6 px-3 mr-3">
        ...
        <div>
          {leftData.map((item) => {
            return <SelectedBox key={item[0].uniqueId} item={item} checked={checked} setChecked={setChecked} />;
          })}
        </div>
      </div>
